...
import { DOMAIN_CLUSTERS, domainsInCluster, NAV_PINS } from "@/lib/nav";
...
const MOTIFS: Record<string, ReactNode> = {
  ...
  environment: (
    <g>
      <path
        d="M4 17c3-8 13-10 16-2-4 1-8 3-10 6-2-2-4-3-6-4z"
        fill="none"
        strokeWidth="1.5"
      />
  ...
  agriculture: (
    <g>
      <path
        d="M12 20V9M12 9c-4 0-6 3-6 6 3 0 5-1 6-3M12 9c4 0 6 3 6 6-3 0-5-1-6-3"
        fill="none"
        strokeWidth="1.5"
      />
...
function DomainTile({ d, i }: { d: DomainMeta; i: number }) {
  return (
    <Link
      href={d.href}
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-3">
        <svg
          ...
          aria-hidden
        >
          {MOTIFS[d.id] ?? <circle cx="12" cy="12" r="6" strokeWidth="1.5" />}
        </svg>
        <span
      ...
      <div className="relative mt-1 text-xs text-[var(--muted)]">{d.titleNp}</div>
...
/** Clustered domain grid — mirrors the nav mental model. */
    ...
    <div className="space-y-8">
        ...
        return (
          <div key={cluster.id}>
            <div className="mb-3 flex items-end justify-between gap-3">
              <div>
                <div className="section-kicker" style={{ color: cluster.accent }}>
                  {cluster.labelNp}
                </div>
                <h3 className="display mt-1 text-xl">{cluster.label}</h3>
                <p className="text-xs text-[var(--muted)]">{cluster.blurb}</p>
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {items.map((d) => (
                <DomainTile key={d.id} d={d} i={i++} />
              ))}
            </div>
          </div>
        );
      ...
      <div>
        <div className="mb-3">
          ...
          <h3 className="display mt-1 text-xl">Map & news</h3>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {NAV_PINS.map((id) => {
            const d = byId.get(id) ?? DOMAINS.find((x) => x.id === id);
            if (!d) return null;
            return <DomainTile key={d.id} d={d} i={i++} />;
          })}
        </div>
      </div>
    </div>
